...
export class Element extends HTMLElement {
  ...
  _getContext() {
    return {
      useEventDispatcher: () => {
        return (type, detail, options) => this.dispatchEvent(new CustomEvent(type, { ...options, detail }));
      },
      ...
      useProperty: (name, model) => {
        ...
        if (model.default && this._properties[name] === undefined) {
  ...
  _update() {
    if (this._render) {
      this._notifySubscribers('lifecycle', 'willUpdate');
